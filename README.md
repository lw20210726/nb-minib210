...
│       ├── Makefile          # 固件 / bootloader 构建入口
│       ├── setup_env.sh      # Cypress FX3 SDK 环境变量配置
│       └── b200/
...
│   ├── README.md             # 适配改动说明（时钟/AD9361 CMOS/约束/构建/验收）
│   ├── build.tcl             # Vivado 批处理构建（时钟变体 × 器件参数化）
│   ├── build.ps1 / build.cmd # Windows 一键构建封装
│   ├── Makefile              # Linux 一键构建封装
│   └── libresdr_b210/        # Vivado 工程（.xpr + srcs）
